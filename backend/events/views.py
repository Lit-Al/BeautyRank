...
from .permissions import IsStaffOrReadOnly, IsMemberOrReadOnly
...
class EventViewSet(ListModelMixin, RetrieveModelMixin, viewsets.GenericViewSet):
    ...
    def winners_nominations(self, request, *args, **kwargs):
        ...
        member_nominations_all = (
            MemberNomination.objects.all().filter(category_nomination__event_category__event=event_id)
            .annotate(result_all=Sum('results__score')).order_by(
                '-result_all'))
        for nomination in nominations:
            ...
            if member_nominations.exists():
                ...
                win_nominations.append({
                    "nomination_or_category": str(nomination),
                    "members": top_three
                })
        print(win_nominations)
        serializer = WinnersSerializer(win_nominations, many=True)
        return Response(
            {"winners_nominations": serializer.data}
        )

    @action(detail=True,
            methods=['get'])  # TODO: Переделать сериализатор, либо разделить это на два action'a. ПОТЕСТИТЬ!
    def winner_with_3_and_2_nom_WANT_REMAKE(self, request, *args, **kwargs):
        ...
        winner_id_for_three = get_sorted_members_for_top3(get_members_gte_3(event_id))[0]
    ...
    def winners_categories(self, request, *args, **kwargs):
        win_categories = []
        event_id = kwargs["pk"]
        categories = Category.objects.filter(categories_in_EventCategoryModel__event=event_id)
        print(categories)
        ...
        return Response(
            {"winners_categories": serializer.data}
        )
